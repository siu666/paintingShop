    ...
    <div class="container">
      <div class="checkout-order">
        <div class="page-title-normal">
          ...
          <!--<router-link class="back_to" to="/dizhi">&#8592</router-link>-->
        </div>
        <!-- process step -->
        ...
        <div class="order-foot-wrap">
          ...
          <div class="next-btn-wrap">
            <button class="btn btn--m btn--red" @click="createOrder" >支付确认</button>
      ...
      getcartList(){
        axios.get("/users/cartList").then((response)=>{
          ...
          this.cartList.forEach((item)=>{
            if(item.selected=='1'){
              this.goodstotal += item.goodsPrice*item.goodsNumber;
            }
          })
          this.dingdantotal =this.goodstotal
        })
      },
    createOrder(){
        var dizhiId = this.$route.query.dizhiId;
          axios.post("/orders/createOrder",{
                dizhiId:dizhiId,
            dingdantotal:this.dingdantotal
          }).then((response)=>{
            let res =response.data;
            if(res.status=="888"){
              this.$router.push({
              name:'orderCreate',params:{'orderId':res.result.orderId}
              })
              console.log(res.result.dingdantotal)
            }
          })
    }
  }
