<template>
    <div>
      ...
      <breadnav><span>购物车</span></breadnav>
                ...
                <li>
                  <div class="cart-tab-1">
                    <div class="cart-item-check">
                      <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.selected==1}" @click="editgwc('selected',item)">
                    ...
                    <div class="cart-item-pic">
                      <img v-bind:src="'/static/'+item.goodsImg">
                    </div>
                    ...
                    <div class="item-quantity">
                      <div class="select-self select-self-open">
                        <div class="select-self-area">
                          <a class="input-sub" @click="editgwc('jian',item)">-</a>
                          <span class="select-ipt">{{item.goodsNumber}}</span>
                          <a class="input-add" @click="editgwc('jia',item)">+</a>
                      </div>
                      </div>
                    </div>
                  </div>
                    ...
                    <div class="cart-item-opration">
                      <a href="javascript:;" class="item-edit-btn">
                        <svg class="icon icon-del">
                          <use xlink:href="#icon-del" @click="showdelMotai(item.goodsId)"></use>
                        </svg>
                      </a>
                    </div>
                  </div>
            ...
            <div class="cart-foot-inner">
              ...
              <div class="cart-foot-r">
                <div class="item-total">
                  购买总价: <span class="total-price">{{cartListtotal}}</span>
                </div>
                <div class="btn-wrap">
                  <a class="btn btn--red" v-bind:class="{'btn--dis':selectedNum==0}" @click="toDiZhi">结算</a>
                </div>
              </div>
            </div>
          </div>
      ...
      <motai :mdShow="deleteMotai" @close="closeModal">
        <p slot="message">确认删除该商品?</p>
        <div slot="btnList">
        <a class="btn btn--m" href="javascript:;" @click="deletegoods">确认</a>

        <a class="btn btn--m" href="javascript:;" @click="deleteMotai = false">关闭</a>
        </div>
      </motai>
...
export default {
  ...
  computed:{
      checkedAllFlag(){
        return this.selectedNum == this.cartList.length;
      },
    selectedNum(){
        let i =0;
    ...
    cartListtotal(){
      ...
      this.cartList.forEach((item)=>{
        if(item.selected == '1')
          total += parseInt(item.goodsNumber)*parseInt(item.goodsPrice)
      })
      return total;
    }
  },
  methods:{
    ...
    showdelMotai(goodsId){
           this.goodsId=goodsId;
          this.deleteMotai=true;
    },
    deletegoods(){
   ...
   /* 之后就是改变当前的选中状态，默认进来的商品数据会在tocart的接口中，给商品的selected字段定义为1（即为选中状态），后续就是对字段中的selected状态的改变，同上，我们可以通过三元表达式去传值，
    从而改变状态值。*/

    toggleCheckedAll(){
        this.ifFlag = !this.checkedAllFlag;
        this.cartList.forEach((item)=>{
          item.selected = this.ifFlag?'1':'0';
        })
      axios.post("/users/editCheckAll",{
        checkAll:this.ifFlag
      }).then((response)=>{
          let res =   response.data;
        if(res.status =='0'){
          console.log("suc")
